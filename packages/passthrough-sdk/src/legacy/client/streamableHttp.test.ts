...
describe("StreamableHTTPClientTransport", () => {
  ...
  it("uses custom fetch implementation if provided", async () => {
    ...
    await (
      transport as unknown as {
        _startOrAuthSse: (opts: StartSSEOptions) => Promise<void>;
      }
    )._startOrAuthSse({});
  ...
  it("should always send specified custom headers", async () => {
    ...
    vi.mocked(global.fetch).mockImplementation(async (_url, reqInit) => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        expect.objectContaining({
          message: expect.stringContaining(
            "SSE stream disconnected: Error: Network failure",
          ),
        }),
      );
