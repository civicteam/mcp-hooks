...
import { type AddressInfo, createServer as netCreateServer } from "node:net";
...
describe("StreamableHTTPServerTransport", () => {
  ...
  it("should send response messages to the connection that sent the request", async () => {
    ...
    if (!reader1) throw new Error("No reader1 available");
    ...
    if (!reader2) throw new Error("No reader2 available");
...
describe("StreamableHTTPServerTransport with resumability", () => {
  ...
  beforeEach(async () => {
    ...
    expect((transport as any)._eventStore).toBeDefined();
  ...
  it("should store and include event IDs in server SSE messages", async () => {
    ...
    if (!idMatch) throw new Error("No match found");
  ...
  it("should store and replay MCP server tool notifications", async () => {
    ...
    if (!reconnectReader) throw new Error("No reconnect reader available");
