...
describe("Ping Handling Tests", () => {
  ...
  describe("Direct echo server connection", () => {
    ...
    it("should track tool call count correctly", async () => {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        expect((result as ToolResult).content[0].text).toBe(
          `Echo: Message ${i}`,
        );
      }
  ...
  describe("Ping handling through passthrough", () => {
    it("should forward pings from target server to client", async () => {
      ...
      client.setRequestHandler(PingRequestSchema, async (request) => {
        console.log(
          "Client received request through passthrough:",
          request.method,
        );
        if (request.method === "ping") {
          pings.push(request);
      ...
      expect((result1 as ToolResult).content[0].text).toBe(
        "Echo: Hello through passthrough",
      );
      ...
      expect((result2 as ToolResult).content[0].text).toBe(
        "Echo: Still working after ping",
      );
    });
    ...
    it("should handle concurrent sessions independently", async () => {
      ...
      try {
        ...
        const result1b = await client1.callTool({
          name: "echo",
          arguments: { message: "Client 1 again" },
        });
        const result2b = await client2.callTool({
          name: "echo",
          arguments: { message: "Client 2 again" },
        });

        expect((result1b as ToolResult).content[0].text).toBe(
          "Echo: Client 1 again",
        );
        expect((result2b as ToolResult).content[0].text).toBe(
          "Echo: Client 2 again",
        );
      } finally {
